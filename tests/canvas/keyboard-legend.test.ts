...
const LANDING = readFileSync(new URL('../../src/pages/forja/index.astro', import.meta.url), 'utf8').replace(
  /<[^>]+>/g,
  '',
)
...
describe('the keyboard legend on /forja', () => {
  ...
  it('names that exact key', () => {
    expect(LANDING).toMatch(new RegExp(`(?<![\\p{L}])${connectKey} para`, 'u'))
  })
  ...
  it('never promises that Enter on its own creates or connects', () => {
    expect(LANDING).not.toMatch(/Enter para crear\/conectar/)
  })
  ...
  it('names the shortcut that opens a component’s or a connection’s menu', () => {
    ...
    expect(LANDING).toContain('Shift+F10')
  })
})
