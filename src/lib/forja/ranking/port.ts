...
// write. Pure types only. No localStorage or DOM here, so this file stays
...
import type { MiniAdr } from '../progression/mastery'

export interface SubmitAttemptInput {
  exerciseId: string
  design: Design
  // RK7: the graph is what gets stored. Score, ceiling and engineVersion ride
...
export type StorageFailureReason = 'quota-exceeded' | 'storage-unavailable' | 'write-failed'

export type StorageWriteStatus =
  | { ok: true; outcome: 'stored' | 'deduplicated'; evicted: number }
  | { ok: false; outcome: 'failed'; reason: StorageFailureReason; evicted: number }

export interface SubmitAttemptResult extends Attempt {
...
export interface RankingPort {
  ...
  // Every attempt including illegal or unscored ones, as personal history,
