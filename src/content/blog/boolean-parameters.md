...
**Por qué funciona:** agregar una opción nueva (`includePdf`, `replyTo`) no cambia ningún call-site existente, porque el campo tiene un default razonable. Los states inválidos se pueden prevenir en el builder. Las combinaciones de dominio viven en un lugar.
...
La señal de alerta en el código es cualquier firma con `boolean` que no sea "¿está habilitado este objeto?", que es una propiedad del objeto y no una instrucción al método. Cuando el booleano le dice al método *cómo comportarse*, el método tiene responsabilidades que no le corresponden.
...
Día 22 de **#100ArchitectureDays**. El código completo, con la implementación ANTES de 4 booleanos, las tres técnicas DESPUÉS, y 20 tests que incluyen el estado inválido, está en el repo.
