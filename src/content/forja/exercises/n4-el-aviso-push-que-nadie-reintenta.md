...
hiddenFacts:
  - fact: el proveedor de push rechaza entre el 2% y el 6% de los envíos por token vencido o app desinstalada. Es un porcentaje chico, pero a 5000/hora son cientos por hora.
...
guarantees:
  ...
  - id: g-end-to-end
    label: el aviso viaja completo desde quien lo publica hasta el proveedor de push
    weight: 2
    predicate:
      op: all
      of:
        ...
        - op: path
          ...
          via:
            type: [worker]
    whyMissing: hay piezas para procesar avisos, pero no un camino completo desde quien publica el aviso hasta el proveedor de push pasando por la cola y el worker.
    consequence: la cola puede parecer atendida aunque ningún aviso real llegue al proveedor. El sistema conserva trabajo que nunca termina de entregar.
  - id: g-has-dlq
    ...
    consequence: un aviso que falla bloquea o se pierde en silencio. Con un solo responsable de guardia, nadie lo va a ver hasta que un usuario reclame.
rubric:
...
referenceSolutions:
  - label: cola con destino para los fallos, un solo cliente de entrada
    contextInversion: la variante más simple, con un solo origen de avisos. Es suficiente si el sistema todavía tiene un único cliente emitiendo notificaciones.
    design:
      nodes:
        - id: app
          type: mobile-client
      ...
      edges:
        ...
        - id: enviador-proveedor
          from: { node: enviador }
          to: { node: proveedor }
  - label: cola con destino para los fallos, más el monitoreo del enviador
    contextInversion: cuando el volumen crece, saber que el enviador está vivo importa tanto como que la cola exista. Esta variante suma observabilidad sobre el propio enviador.
    design:
      nodes:
        - id: app
          type: mobile-client
...
El equipo tiene **6 unidades operativas** de presupuesto y no está pidiendo
