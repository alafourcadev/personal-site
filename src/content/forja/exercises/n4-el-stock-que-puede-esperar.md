...
guarantees:
  - id: g-decoupled
    ...
    consequence: en el pico de Black Friday, una llamada directa satura al servicio de inventario y arrastra al checkout con él. Y si se pierde el aviso, el stock queda mal contado hasta la próxima auditoría.
  - id: g-observability
    ...
    design:
      ...
      edges:
        ...
        - id: checkout-obs
          from: { node: checkout }
          to: { node: obs }
  - label: registro de eventos de venta
    contextInversion: un registro de eventos conviene cuando otro sistema (analítica de demanda, reposición automática) también necesita leer el mismo evento de venta, no sólo inventario.
    design:
      nodes:
        - id: comprador
          type: actor
