...
referenceSolutions:
  - label: cola con destino para los fallos
    contextInversion: la cola es la elección clásica cuando el equipo ya opera colas — una pieza más para monitorear, a cambio de un componente conocido.
    design:
      nodes:
        - id: huesped
          type: actor
      ...
      edges:
        ...
        - id: reservas-obs
          from: { node: reservas }
          to: { node: obs }
  - label: stream de eventos de reserva
    contextInversion: un stream tiene sentido cuando otro sistema (facturación, analítica de ocupación) también necesita leer el mismo evento de pedido de reserva.
    design:
      nodes:
        - id: huesped
          type: actor
        ...
        - id: eventos
          type: stream
          label: Stream de pedidos de reserva
          zone: private
