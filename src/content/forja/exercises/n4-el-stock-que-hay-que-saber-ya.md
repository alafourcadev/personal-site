...
guarantees:
  - id: g-immediate-answer
    ...
    predicate:
      ...
      forbid:
        type: [queue, stream]
    whyMissing: no hay un camino directo (sin cola ni registro de eventos en el medio) desde el checkout hasta el inventario.
...
referenceSolutions:
  - label: llamada directa a inventario
    contextInversion: la variante mínima, con un solo salto síncrono. Suficiente cuando el checkout sólo necesita esa respuesta y nada más.
    design:
      nodes:
        - id: comprador
          type: actor
