    ...
    design:
      ...
      edges:
        ...
        - id: pagos-obs
          from: { node: pagos }
          to: { node: obs }
  - label: registro de eventos de compra
    contextInversion: un registro de eventos tiene sentido cuando el equipo ya tiene otros consumidores que necesitan leer el mismo evento de compra (analítica, facturación). Un evento por su naturaleza se puede releer, una cola normalmente no.
    design:
      nodes:
        - id: cliente
          type: actor
...
presupuesto operativo del equipo es de **6 unidades operativas**: cada
pieza nueva del sistema (una cola, un proceso de fondo, un componente de
monitoreo) cuenta contra ese número, y por encima de él el equipo deja de
poder sostener lo que armó.
