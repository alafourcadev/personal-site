...
const TAG_ROWS: { id: string; icon: LucideIcon; label: string }[][] = [
  [
    { id: "java", icon: Code, label: "Java" },
    { id: "spring", icon: Layers, label: "Spring Boot" },
    { id: "postgres", icon: Database, label: "PostgreSQL" },
    { id: "docker", icon: Container, label: "Docker" },
    { id: "aws", icon: Cloud, label: "AWS" },
  ],
  [
    { id: "k8s", icon: Network, label: "Kubernetes" },
    { id: "kafka", icon: Server, label: "Kafka" },
    { id: "microservices", icon: Cpu, label: "Microservicios" },
    { id: "rest", icon: Globe, label: "REST API" },
    { id: "git", icon: GitBranch, label: "Git" },
  ],
  [
    { id: "cicd", icon: Settings, label: "CI/CD" },
    { id: "ddd", icon: Layers, label: "DDD" },
    { id: "tdd", icon: Shield, label: "TDD" },
    { id: "typescript", icon: Code, label: "TypeScript" },
    { id: "linux", icon: Terminal, label: "Linux" },
  ],
  [
    { id: "terraform", icon: Cloud, label: "Terraform" },
    { id: "redis", icon: Database, label: "Redis" },
    { id: "graphql", icon: Globe, label: "GraphQL" },
    { id: "security", icon: Lock, label: "Security" },
    { id: "monitoring", icon: Monitor, label: "Observability" },
  ],
];
              ...
              {TAG_ROWS.map((row, rowIndex) => (
                ...
                >
                  {[...row, ...row, ...row].map((item, idx) => (
                    <div
                      key={`${item.id}-${idx}`}
                      className="flex gap-2 bg-[#131b2e]/80 backdrop-blur-sm whitespace-nowrap w-fit text-[#94a3b8] p-2 px-3 items-center border border-[#1e2d4a] rounded-full text-xs"
                    >
                      <item.icon size={14} />
